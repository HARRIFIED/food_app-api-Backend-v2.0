...
router.delete("/delete/:id", verifyTokenAndAuthorization, async (req, res) => {
    try{
        ...
        res.status(200).json({alert: `${req.user.id} sucessfully deleted`})
    } catch (err) {
        res.status(500).json({alert: `something went wrong ${err}`})
    }
});

//TO BE USED FOR ADMIN DASHBOARD
...
//GET ALL USERS
router.get("/query/all/", verifyTokenAndAdmin, async (req,res) => {
    try{
        const query = req.query.new;
        const users = query ? await User.find().sort({_id: -1}).limit(1) : await User.find().sort({_id: 1});
